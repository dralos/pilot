<template>
  ...
  <v-card :loading="loading" class="mx-auto my-12" max-width="374">
    ...
    <v-rating
      :model-value="cartProduct.rating.rate"
      color="white"
    ...
    <v-card-actions>
      <v-chip-group>
        ...
        <v-chip>{{
          toCurrency(cartProduct.quantity * cartProduct.price)
        }}</v-chip>
      </v-chip-group>
      <v-btn
        color="deep-purple-lighten-2"
        variant="text"
        @click.prevent="cartStore.remove(cartProduct.id)"
      >
      ...
      <v-btn
        color="deep-purple-lighten-2"
        variant="text"
        @click.prevent="cartStore.add(cartProduct.id)"
      >
